...
V1_TIMELINE_JSON="$ARTIFACT_DIR/v1_timeline.json"
V1_INCIDENTS_JSON="$ARTIFACT_DIR/v1_incidents.json"
...
curl -fsS --max-time 3 "$API_BASE/v1/timeline?limit=5" >"$V1_TIMELINE_JSON"
curl -fsS --max-time 3 "$API_BASE/v1/incidents?limit=5" >"$V1_INCIDENTS_JSON"

echo "$health_payload" | grep -q '"status":"ok"'
echo "$metrics_payload" | grep -q "flowforge_uptime_seconds"
grep -q '^\[' "$TIMELINE_JSON"
grep -q '"items"' "$V1_TIMELINE_JSON"
grep -q '"has_more"' "$V1_TIMELINE_JSON"
grep -q '"items"' "$V1_INCIDENTS_JSON"
grep -q '"has_more"' "$V1_INCIDENTS_JSON"
...
echo "- v1 timeline payload: $V1_TIMELINE_JSON"
echo "- v1 incidents payload: $V1_INCIDENTS_JSON"
